...
@section('title', 'Supplier - Material Control System')
...
<div class="mb-6 px-4 md:px-6">
    <div class="flex gap-2 items-center">
        <!-- Search Form -->
        <form action="{{ route('suppliers.index') }}" method="GET" class="flex gap-1 flex-1 max-w-[40%] sm:max-w-xs lg:max-w-md">
            ...
            @if(request('search'))
                <a href="{{ route('suppliers.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-3 lg:px-4 py-2 rounded flex-shrink-0" title="Reset">
        ...
        <div class="flex gap-1 sm:gap-1.5 lg:gap-2 flex-shrink-0">
            <a href="{{ route('suppliers.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 lg:px-4 py-2 rounded inline-flex items-center" title="Tambah Supplier">
                <i class="fas fa-plus"></i>
                <span class="hidden lg:inline lg:ml-2">Tambah Supplier</span>
            </a>
            ...
            <a href="{{ route('suppliers.export') }}" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 lg:px-4 py-2 rounded inline-flex items-center" title="Export Excel">
                <i class="fas fa-file-excel"></i>
                <span class="hidden lg:inline lg:ml-2">Export Excel</span>
            </a>
            <a href="{{ route('suppliers.pdf') }}" target="_blank" class="bg-red-600 hover:bg-red-700 text-white px-3 lg:px-4 py-2 rounded inline-flex items-center" title="Export PDF">
...
<div id="importModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-semibold">Import Suppliers dari Excel</h3>
        ...
        <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
            ...
            <a href="{{ route('suppliers.template') }}" class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800 font-semibold">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
            <div class="p-3">
                <div class="flex justify-between items-start mb-2">
                    <div class="flex-1">
                        <h3 class="font-semibold text-xs text-gray-900 line-clamp-2">{{ $supplier->supplier_name }}</h3>
                        <p class="text-xs text-gray-500 mt-1">{{ $supplier->supplier_code }}</p>
                    </div>
                ...
                <div class="space-y-1 text-xs mb-2">
                    <div class="flex justify-between">
                        ...
                        <span class="text-gray-900 font-medium truncate ml-1">{{ $supplier->contact_person ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Phone:</span>
                        <span class="text-gray-900 font-medium">{{ $supplier->phone ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">City:</span>
                        <span class="text-gray-900 font-medium">{{ $supplier->city ?? '-' }}</span>
                    </div>
                </div>
                <div class="flex justify-end space-x-2 pt-2 border-t border-gray-200">
                    <a href="{{ route('suppliers.edit', $supplier) }}" class="text-yellow-600 hover:text-yellow-800">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">{{ $supplier->supplier_code }}</td>
                    <td class="px-6 py-4 text-sm">{{ $supplier->supplier_name }}</td>
                    <td class="px-6 py-4 text-sm">{{ $supplier->contact_person ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm">{{ $supplier->phone ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm">{{ $supplier->city ?? '-' }}</td>
                    ...
                    <td class="px-6 py-4 text-center whitespace-nowrap text-sm">
                        <a href="{{ route('suppliers.edit', $supplier) }}" class="text-yellow-600 hover:text-yellow-900 mr-3">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('suppliers.destroy', $supplier) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                            @csrf
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-6 py-4 text-center text-gray-500">Belum ada data supplier</td>
                </tr>
                @endforelse
            </tbody>
        </table>
